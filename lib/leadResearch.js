...
function formatOwnerName(raw) {
  ...
  // Skip generic placeholder strings
  if (/^(recorded owner|owner|unknown|n\/a|none|trust|llc|corp|inc|lp|ltd)/i.test(s)) return null
  // If already mixed case (not all-caps), return as-is
  if (s !== s.toUpperCase()) return s
  // Convert all-caps assessor format: "SMITH JOHN" or "SMITH JOHN A"
  const parts = s.split(/\s+/).filter(Boolean)
  if (parts.length >= 2) {
    // Assessor format is typically LAST FIRST [MIDDLE]
    const last = parts[0]
...
function buildRentCastNormalizedLead(record, filters) {
  ...
  return scoreNormalizedLead(createNormalizedLead({
    ...
    contact: {
      name: formatOwnerName(ownerNames[0]) || formatOwnerName(ownerNames[1]) || 'Owner on Record',
      phones: [],
      emails: [],
      preferredChannel: 'Skip Trace',
    },
    location: {
...
export async function researchLeads(filters) {
  ...
  // Even when RentCast fills the quota, still run targeted contact searches
  // so we can find phone numbers for owners (property records never include contact info)
  const rentcastFull = directProviderLeads.length >= desiredCount
  if (rentcastFull && !process.env.SERPAPI_API_KEY && !process.env.BING_SEARCH_KEY) {
    // No web search keys configured — return RentCast data as-is
    ...
    return {
      ...
      meta: {
        ...
        contactNote: 'Contact info requires skip tracing — phone/email not available in property records.',
      },
    }
  }
